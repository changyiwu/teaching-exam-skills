...
讀取 `references/試卷雙向細目分析表(空白表).doc` 取得格式規範，依下列結構規劃：
...
**各層次出題原則**（參考 `references/布魯姆分類學.pdf`）：
...
讀取 `references/布魯姆分類學.pdf` 的詳細分級準則，對每道題目判定：
...
| 需要什麼 | 讀取哪個檔案 |
|----------|-------------|
| Bloom 四級詳細準則、關鍵字、出題原則 | `references/布魯姆分類學.pdf` |
| 雙向細目表格式規範 | `references/試卷雙向細目分析表(空白表).doc` |
| 題目卷 / 答案卷版面格式 | `references/試卷範例.docx` |
| 108課綱三年數學完整章節、學習內容代碼（N/A/S/F/D）、學習表現代碼 | `references/康軒-114中數1-3年級課程架構表(114年入學適用).doc` |
